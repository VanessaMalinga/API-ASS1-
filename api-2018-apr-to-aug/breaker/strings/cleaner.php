...
";?>
<?php
$wce = explode(" ",$wambua_corrupted_employees);?>
<?
function clean ($wambua_corrupted_employees)
{
  //code comes here
  ?>
<table style="border:1px solid black; padding:1px">
  <thead>
  <?php //for ($i=0; $i <8 ; $i++) {
    $wambua_corrupted_employees = preg_replace('/\d+/', '', $wambua_corrupted_employees);//this removes numbers ?>

    <th style="border:1px solid black;><?php echo (preg_replace('/[()0-9>+=\/]+/', '',ucwords(strtolower($wce[$i])))) ; ?> </th>

  <?php
   $wambua_corrupted_employees = preg_replace('/[^A-Za-z0-9\-]/','' ,$wambua_corrupted_employees); // this removes special chars.
  ?>

  <?php //damn this teacher is evilly educating ?>
...
<?php clean($wambua_corrupted_employees) ?>
